...
/*COURSES*/
$stmt = $conn->prepare("select account_type,account_name,first_name,last_name,account_password,description from accounts a , accounttype act where a.account_id = act.account_type_id;");
$stmt->execute();
$json = $stmt->fetchAll(PDO::FETCH_ASSOC);
$json = array_values($json);
...
/*ACCOUNTS*/
$stmt = $conn->prepare("select f.faculty_category,st.school_type,shfc.year_of_study,shfc.mean_salary,f.faculty_name,s.school_name,c.course_name,shfc.course_fee from schooltype st,school_have_faculty_course shfc, school s, course c, faculty f where shfc.school_id = s.school_id and shfc.course_id = c.course_id and shfc.faculty_id = f.faculty_id and st.school_type_id=s.school_type_id");
$stmt->execute();
$json = $stmt->fetchAll(PDO::FETCH_ASSOC);
$json = array_values($json);
