...
/*COURSES*/
$stmt = $conn->prepare("select shfc.year_of_study,shfc.mean_salary,f.faculty_name,f.faculty_category,st.school_type,s.school_name,c.course_name,shfc.course_fee from school_have_faculty_course shfc, school s, course c, faculty f, schooltype st where s.school_type_id = st.school_type_id and shfc.school_id = s.school_id and shfc.course_id = c.course_id and shfc.faculty_id = f.faculty_id");
$stmt->execute();
$json = $stmt->fetchAll(PDO::FETCH_ASSOC);
$json = array_values($json);
